...
class Magenest_Learn_Main {
    ...
    public function __construct() {
      ...
      //  add_action('init',array($this,'register_session'));
        ...
        add_action( 'wp_ajax_take_quizz_answer',  array($this, 'take_quizz_event_json') );
    ...
    public function take_quizz_event_json() {
        $answerId = $_REQUEST['answerId'];
        $is_correct = $_REQUEST['is_correct'];

        $answerData = Magenest_Learn_Question::getAnswerById($answerId);
        $history = $answerData[0]['history'];
        $historyArr =[];
        if ($history) {
            $historyArr = unserialize($history);
            $todayStr = date("Y.m.d H:i");
            if ($is_correct) {
                $historyArr[$todayStr] = 'correct';
            } else {
                $historyArr[$todayStr] = 'wrong';
            }
        } else {
        ...
        $wrong_count = $answerData[0]['wrong_count'];
        $correct_count = $answerData[0]['correct_count'];
        $value=[];
        ...
        $value['wrong_count'] = $wrong_count;
        $value['id'] = $answerId;
        $value['correct_count'] = $correct_count;
        $value['history'] = serialize($historyArr);
        $value['quizz_at'] = date('Y-m-d H:i:s')
        ;
