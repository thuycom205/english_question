...
class Magenest_Learn_Question {
    ...
    public static function updateQuestionP($data) {
        global $wpdb;
        $prefix = $wpdb->prefix;
        $tbl = "{$prefix}magenest_learn_answer";
        $wpdb->update($tbl,$data, array('post_id' =>$data['id']));
    }
    ...
    public static function getAnswerById($postId) {
        global $wpdb;
        $prefix = $wpdb->prefix;
        $tbl = "{$prefix}magenest_learn_answer";

        //query
        $query = "select * from {$tbl} where id={$postId}";
        $rows = $wpdb->get_results($query,ARRAY_A);
        return $rows;
    }
