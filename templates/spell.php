...
<div id="learn-answer">
    <div data-bind="foreach:answers">
        ...
        <div class ="check-answer tbc"><span data-bind="click:sound"> Sound</span> </div>
...
<script type="text/javascript">
        ...
        this.check = function(answer, event) {
            if (compareStrings(answer.en(),answer.enAnswer() , true, false) ) {
              ...
              //  playSoundplaySound('<?php echo LEARN_URL?>/audio/' + answer.en() +'__us_1.mp3');
                ...
                answer.correct(true);
                answer.wrong(false);
                updateQuizzResult('<?php  echo $adminURL?>' , answer.id(), 1);
            } else {
               ...
               // playSound('<?php echo LEARN_URL?>/audio/wrong__us_1.mp3');
                ...
                answer.wrong(true);
                updateQuizzResult('<?php  echo $adminURL?>' , answer.id(), 0);

            }
        }

        this.sound = function(answer, event) {
            if (compareStrings(answer.en(),answer.enAnswer() , true, false) ) {
    ...
    function updateQuizzResult(url,answerId , is_correct) {
        jQuery.post( url,
            {
            action :'take_quizz_answer',
            answerId: answerId,
            is_correct: is_correct
            })
            .done(function( data ) {
                console.log('update ket qua');
            });
    }
    function ViewModel() {
        // Data
